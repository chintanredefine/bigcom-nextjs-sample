...
const ProductOptions: React.FC<ProductOptionsProps> = React.memo(
  ({ options, selectedOptions, setSelectedOptions, product, variants, currentVariant, skuChange }) => {
    ...
    function handleSearchModal(input: any, ul: any, li: any, f: any, ftype: any) {
      var filter,a, i, txtValue;
    ...
    function filterInStockOptions(element: any, stockEvent: any, ul: any, li: any, f: any) {
        var i;
    ...
    function filterOutOfStockOptions(element: any, stockEvent: any, ul: any, li: any, f: any) {
        var i;
    ...
    function changeToGidModal(element: any, x: any, y: any) {
      element = document.getElementById("all_options_list");
      element.classList.remove("list");
      element.classList.add("grid");
    ...
    function modalClose(element: any, x: any) {
      element = document.getElementById("all_options_list");
    ...
    function modalCloseInner(element: any, x: any, y: any) {
      element = document.getElementById("selected_color_options");
      element.style.display = "none";
      x = document.getElementById("disabled_color_options");
      x.style.display = "none";

      y = document.getElementById("all_options_list");
      y.classList.remove("modal_option_fix");
    }

    function pickColor(va: any, x: any, y: any, cimg: any, element: any, z: any){
      let sku = (va.sku).toLowerCase()
      ...
      let colorimg = publicRuntimeConfig.COLOR_SWATCH_URL + "/stencil/276x342/attribute_value_images/"+cid+".preview.jpg"
          ...
          <div className="pb-6 grid" key="Options" id="options_list">
            <div className="flex justify-between">
                <h2 className="uppercase font-medium text-sm tracking-wide">
                  Color <span className="option-title"> {currentVariant.option_values ? currentVariant.option_values[0].label : ''} </span>
                </h2>
                <span id="shade_search_block">
                  <input type="text" id="shade_search" placeholder="Shade Search" onKeyUp={(event) => {handleSearch(event,null,null)}} />
                </span>
                <div className="mobile_options">
                  <span className="grid_btn" onClick={(event) => {changeToGid(event)}}>Grid</span>
                  <span className="seperator">/</span>
                  <span className="list_btn" onClick={(event) => {changeToList(event)}}>List</span>
                </div>
            </div>
            <div className="option-section">
              ...
              <input id="selected_variant" type="hidden" value={currentVariant.id} />
                {variants.data.map((v: any, i: number) => {
                  ...
                  const swatch = publicRuntimeConfig.COLOR_SWATCH_URL + "/stencil/96x120/attribute_value_images/"+opt.id+".preview.jpg"
                ...
                {totalVariants > 17 ? (
                  <div className="moreOption expanderButton view-more-section innerOption" id="view_more">
                    <div className="a-section a-spacing-none expanderBullet" onClick={(event) => {viewMoreShow(event,null)}}>
                      View All<br /> Shades
                    </div>
                  </div>
                 ) : ''
            ...
            <div className="pb-6 grid options_modal" key="Options" id="all_options_list" style={{ display: "none" }}>
              <div className="flex justify-between items-baseline headModal">
                  <h2 className="uppercase font-medium text-sm tracking-wide">
                    Color
                  </h2>
                  <div className="filter_middle">
                    <button className="in-stock-btn active" id="in_stock_filter" aria-label="In Stock" type="button" onClick={(event) => {filterInStockOptions(event,null,null,null,null)}}>
                    In Stock</button>
                    <button className="out-of-stock-btn" id="out_of_stock_filter" aria-label="Out Of Stock" type="button" onClick={(event) => {filterOutOfStockOptions(event,null,null,null,null)}}>
                    Out Of Stock</button>
                  </div>
                  <span id="shade_search_block">
                    <input type="hidden" id="filter_type" defaultValue="instock" />
                    <input type="text" id="shade_search_modal" placeholder="Shade Search" onKeyUp={(event) => {handleSearchModal(event,null,null,null,null)}} />
                  </span>
                  <span className="modal_close_btn cursor-pointer" onClick={(event) => {modalClose(event, null)}}><Cross /></span>
                  <div className="mobile_options">
                    <span className="grid_btn active" id="modal_grid_btn" onClick={(event) => {changeToGidModal(event,null,null)}}>Grid</span>
                    <span className="seperator">/</span>
                    <span className="list_btn" id="modal_list_btn" onClick={(event) => {changeToListModal(event,null,null)}}>List</span>
                  </div>
              </div>
              <div className="option-section">
                <div id="alloptions_modal" className="flex_div flex justify-start flex-wrap flex-row py-4">
                  ...
                  {variants.data.map((v: any, i: number) => {
                    ...
                    return (
                      <Swatch
                        ...
                        label={opt.label}
                        className={customclass + isDisabled}
                        onClick={(event) => {pickColor(v,null,null,null,null,null)}}
                      />
                    )
                  })}
                </div>
              ...
              <div className="selected_color_options" id="selected_color_options" style={{ display: "none" }}>
                <h4>Color Selected</h4>
                <span className="modal_close_btn cursor-pointer" onClick={(event) => {modalCloseInner(event, null, null)}}><Cross /></span>
                <div className="option-title" id="selected_color_title">{currentVariant.option_values ? currentVariant.option_values[0].label : ''}</div>
                <div className="option-sku" id="selected_color_sku">SKU: {currentVariant.sku}</div>
                <img  className="option-img" src={currentVariant.image_url} id="selected_color_img" />
                ...
                <div className="add_to_cart">
                  ...
                  <div className="option-price" id="selected_color_price">${currentVariant.price}</div>
              ...
              <div className="selected_color_options out_of_stock_modal" id="disabled_color_options" style={{ display: "none" }}>
                <h4>Be Notified</h4>
                <span className="modal_close_btn cursor-pointer" onClick={(event) => {modalCloseInner(event, null, null)}}><Cross /></span>
                <div className="name_img">
                  <div className="option-title" id="disabled_color_title">{currentVariant.option_values ? currentVariant.option_values[0].label : ''}</div>
                  <img  className="option-img" src={currentVariant.image_url} id="disabled_color_img" />
                </div>
                <div className="add_to_cart">
                  <div className="wishlist"> 
                    {process.env.COMMERCE_WISHLIST_ENABLED && (
                ...
                <div className="notify-section">
                  <form className="notify-form">
                    <input type="text" placeholder="Youremail@mail.com" className="form-input" />
                    <button type="submit" placeholder="Youremail@mail.com" className="form-input">Notify Me When Available</button>
