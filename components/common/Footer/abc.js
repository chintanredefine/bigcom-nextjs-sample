    ...
    (window.autoSuggestObj = c.extend(s.prototype, {
      ...
      getPopularProductsHeader: function (t) {
        var s = t.options.popularProducts.header,
          e = ''
        return (
          'string' == typeof s ? (e = s) : 'function' == typeof s && (e = s(t)),
          e
        )
      },
      init: function (t, s) {
        ...
        ;(this.options = c.extend({}, this.default_options, s)),
          ...
          (this.$results = c('<div/>', {
            class: this.options.resultsClass + ' unbxd-as-overall-autosuggest',
          })
            .css(
              'position',
      ...
      showResults: function () {
        ...
        var o =
          ...
          i = (parseInt(t.css('padding-bottom'), 10), parseInt(o) - 2 + a + i),
          n = {
        ...
        this.$results.find('ul.unbxd-as-maincontent').css('width', i + 'px'),
          ...
          !jQuery('body').hasClass('unbxd-body-overlay') &&
            1024 < screen.width &&
            jQuery('body').addClass('unbxd-body-overlay'),
      ...
      hideResultsNow: function () {
        this.log('hideResultsNow'),
          ...
          this.$results.is(':visible') &&
            (this.$results.hide(),
            jQuery('body').removeClass('unbxd-body-overlay')),
      ...
      getfilteredPopularProducts: function () {
        ...
        this.currentResults))
          if ('POPULAR_PRODUCTS' != a && this.currentResults.hasOwnProperty(a))
            for (var l in this.currentResults[a])
              this.currentResults[a].hasOwnProperty(l) &&
                ((n = this.currentResults[a][l].filtername
                  ? i +
                    '?q=' +
                    encodeURIComponent(this.currentResults[a][l].autosuggest) +
                    '&filter=' +
                    this.currentResults[a][l].filtername +
                    ':"' +
                    encodeURIComponent(this.currentResults[a][l].filtervalue) +
                    '"&rows=' +
                    this.options.popularProducts.count +
                    s +
                    '&' +
      ...
      setDefaultOptions: function () {
        this.options.inFields.type || (this.options.inFields.type = 'separate'),
      ...
      highlightStr: function (t) {
        ...
        if (e.indexOf(' ')) {
          ...
          for (i in n)
            n.hasOwnProperty(i) &&
              (-1 != (o = s.toLowerCase().lastIndexOf('</strong>')) && (o += 9),
              (s =
                s.substring(0, o) +
                s
                  .substring(o)
      ...
      prepareinFieldsHTML: function () {
        return 'inline' === this.options.inFields.type
          ? '{{#if data.IN_FIELD}}' +
              ...
              (this.options.inFields.tpl || this.default_options.inFields.tpl) +
              '</li>{{/unbxdIf}}{{/each}}{{/if}}'
          : '{{#if data.IN_FIELD}}' +
              (this.options.inFields.header
                ? '<li class="unbxd-as-header">' +
      ...
      preparetopQueriesHTML: function () {
        return (
          ...
          (this.options.topQueries.tpl || this.default_options.topQueries.tpl) +
          '</li>{{/each}}{{/if}}'
        )
      },
...
unbxdAutoSuggestFunction(jQuery, Handlebars),
  (unbxd_as_config = {
    ...
    delay: 100,
    loadingClass: 'unbxd-as-loading',
    mainWidth: 0.158 * jQuery('.Navbar_root__2kbI9').outerWidth(),
    sideWidth:
      jQuery('.Navbar_root__2kbI9').outerWidth() -
      0.157 * jQuery('.Navbar_root__2kbI9').outerWidth() -
      130,
    zIndex: 9999,
    position: 'absolute',
    sideContentOn: 'right',
        ...
        Handlebars.registerHelper('productPrice', function (t, s, e, i) {
          ...
          return (
            t == e && s == i && s == e
              ? (console.log('1'),
                (o +=
                  '<div class="black-color-price">$' + t.toFixed(2) + '</div>'))
              : t == e && s == i && e != i
              ? (console.log('2'),
                (o +=
    ...
    noResultTpl: function (t) {
      return (
        jQuery('.unbxd-as-maincontent li').css('width', '70% !important'),
        jQuery('.unbxd-as-maincontent li').css('margin-left', '0px !important'),
    ...
    processResultsStyles: function (t) {
      return (
        0 == jQuery('.unbxd-as-maincontent').length &&
          jQuery('.unbxd-as-sidecontent').css('margin-left', '130px'),
        screen.width < 1024
          ? 0 < jQuery('#mobile-search').val().length
            ? jQuery('.unbxd-as-wrapper').css({ visibility: 'visible' })
            : jQuery('.unbxd-as-wrapper').css({ visibility: 'hidden' })
          : 0 < jQuery('#search').val().length
          ? jQuery('.unbxd-as-wrapper').css({ visibility: 'visible' })
          : jQuery('.unbxd-as-wrapper').css({ visibility: 'hidden' }),
        (t.left = 'auto'),
        1023 < screen.width &&
  ...
  (screen.width < 1024
    ? jQuery('#mobile-search')
    : jQuery('#search')
  ).unbxdautocomplete(unbxd_as_config),
  jQuery(document).ready(function () {
    jQuery('.mobileMenu-toggle').after(
      ...
      jQuery('.Navbar_logocustom__3e0xJ img').addClass('logo-image')
  }),
  jQuery(document).on('click', '#search', function () {
    0 == jQuery('.Searchbar_iconContainer_close').length &&
      jQuery(this)
      ...
      jQuery('body').addClass('unbxd-body-overlay'),
      jQuery('.Searchbar_iconContainer_close').css('display', 'flex'),
      jQuery('.unbxd-as-wrapper').css('visibility', 'visible')
  }),
  jQuery(document).on('click', '.Searchbar_iconContainer_close', function () {
    console.log('hii'),
      jQuery(this)
        .closest('.Searchbar_rootcustom__1dui-')
  ...
  jQuery(document).on('click', '#mobile-search', function () {
    0 == jQuery('.flex.pb-4 .Searchbar_iconContainer_close').length &&
      jQuery(this)
        .closest('.Searchbar_rootcustom__1dui-')
        .append('<div class="Searchbar_iconContainer_close">X</div>'),
      jQuery('.Searchbar_iconContainer_close').css('display', 'flex')
  }),
  jQuery(document).on('click', '.click-icon-search', function () {
    jQuery('.flex.pb-4 .Searchbar_rootcustom__1dui-').addClass('open-mobile'),
      jQuery('.flex.items-center.justify-end').css('display', 'none'),
      jQuery('.flex.pb-4 .Searchbar_rootcustom__1dui-.open-mobile').css(
        'display',
        'block'
      )
  }),
  jQuery(document).on(
    ...
    function () {
      jQuery('.flex.pb-4 .Searchbar_rootcustom__1dui-').removeClass(
        'open-mobile'
      ),
        jQuery('.flex.items-center.justify-end').css('display', 'flex'),
        jQuery('.flex.pb-4 .Searchbar_rootcustom__1dui-').css('display', 'none')
    }
  ),
  jQuery('body').append(
  ...
  jQuery(document).on('click', '.filter-mob-buttons .sort-btn', function () {
    jQuery(this)
      .closest('.filter-mob-buttons')
      .find('.unbxd-sortby')
      .toggleClass('dynamic-sort')
  }),
  jQuery(document).on('click', '.unbxd-sort-container', function () {
    jQuery(this).find('.unbxd-sortby').toggleClass('dynamic-sort'),
      jQuery(this)
